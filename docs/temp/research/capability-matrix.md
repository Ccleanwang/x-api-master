...
标记：`✅` 已发现相关实现；`⚠️` 有线索但能力边界待确认；`❓` 本轮未确认；`自研` 暂不应直接下结论。

| 能力 | One API | New API | LiteLLM | 源码/配置依据与备注 |
| --- | --- | --- | --- | --- |
| OpenAI 兼容接口 | ✅ | ✅ | ✅ | One/New 的 `relay/` 适配层；LiteLLM README 与 Proxy 入口 |
| vLLM / OpenAI 兼容上游 | ⚠️ | ⚠️ | ✅ | LiteLLM README 明确列出 vLLM；One/New 需确认渠道配置 |
...
| 团队/角色/细粒度权限 | ⚠️ | ✅ | ⚠️ | New API 有 `controller/channel_authz.go` 及对应测试；One API/LiteLLM 仍需核对具体权限模型 |
| 速率限制 | ✅ | ✅ | ✅ | One/New `middleware/rate-limit.go`、`common/rate-limit.go`；LiteLLM Proxy 配置/Router |
| 并发限制 | ⚠️ | ✅ | ⚠️ | New API 有 `common/limiter/`、Lua 限流脚本和模型限流中间件；One API 仅初步发现 rate-limit |
| 上下文长度限制 | ❓ | ⚠️ | ⚠️ | New API 有 `request_body_limit.go` 和计数测试，但 Token/context 语义仍需确认；LiteLLM 有 model limits 线索 |
...
| 多节点部署 | ⚠️ | ⚠️ | ⚠️ | One/New Compose 有多节点环境变量；LiteLLM 仓库含部署/Redis/Helm 线索，具体一致性待确认 |
